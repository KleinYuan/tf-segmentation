...
class SegApp(object):
    ...
    # Also do undistort if necessary
    @staticmethod
    def _undistort_frame(img, K, D):
        h, w = img.shape[:2]
        new_cam, _ = cv2.getOptimalNewCameraMatrix(cameraMatrix=K, distCoeffs=D,imageSize=(w, h), alpha=0)
        return cv2.undistort(img, K, D, dst=None, newCameraMatrix=new_cam)
